...
def test_detection_config_is_immutable_forbids_unknowns_and_serializes_canonically() -> None:
    ...
    with pytest.raises(ValidationError):
        DetectionAnalysisConfig(
            instrument_id="US30",
            calendar_id="cal",
            made_up=True,  # type: ignore[call-arg]
        )
...
def test_pattern_defaults_are_fully_resolved_before_config_snapshot() -> None:
    ...
    expected = PatternSelection.from_definition(
        definition,
        {"bars": 3},
    )
    assert PatternSelection.from_definition(definition) == expected


def test_evaluation_plan_changes_do_not_mutate_detection_config() -> None:
    detection = DetectionAnalysisConfig(
        instrument_id="US30",
        calendar_id="cal",
    )
    ...
    plan_a = EvaluationPlan(
        ...
        outcomes=(
            OutcomeSelection(
                outcome_id="return",
                outcome_version="1",
                parameters=(ConfigParameter(name="bars", value=5),),
            ),
        ),
    )
    plan_b = EvaluationPlan(
        ...
        outcomes=(
            OutcomeSelection(
                outcome_id="return",
                outcome_version="1",
                parameters=(ConfigParameter(name="bars", value=10),),
            ),
        ),
        segments=(
            SegmentSelection(
...
def test_decimal_timestamp_and_preset_revision_semantics() -> None:
    ...
    plan = EvaluationPlan(
        ...
        export_settings=(
            ConfigParameter(
                name="when",
                value=datetime(2026, 1, 1, 2, tzinfo=local),
            ),
    ...
    preset = ConfigurationPresetRevision(
        preset_id="default",
        revision=2,
        instrument_id="US30",
...
def test_blank_identifiers_and_duplicate_segment_parameters_are_rejected() -> None:
    ...
    with pytest.raises(ValidationError, match="segment parameter"):
        SegmentSelection(
            ...
            segment_version="1",
            parameters=(
                ConfigParameter(name="x", value=1),
                ConfigParameter(name="x", value=2),
            ),
        )
