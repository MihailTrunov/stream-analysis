...
class ParameterSpec:
    ...
    def __post_init__(self) -> None:
        ...
        lo, hi = _decimal_or_none(self.minimum), _decimal_or_none(self.maximum)
    ...
    def canonical(self, *, include_default: bool, include_description: bool) -> dict[str, object]:
...
class ConditionGroup:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.group_id.strip() or not values or any(not value.strip() for value in values):
...
class PatternDefinition:
    ...
    def __post_init__(self) -> None:
        if any(not getattr(self, name).strip() for name in ("pattern_id", "pattern_version", "name", "description")):
            raise PatternDefinitionError("pattern identity/name/description must be non-empty")
        for name in (
            "required_components", "required_market_events", "parameters", "lifecycle_states",
            "transitions", "condition_groups", "simultaneous_precedence", "context_schema",
        ...
        _unique("parameter ids", (value.parameter_id for value in self.parameters))
        _unique("lifecycle states", self.lifecycle_states)
        _unique("condition group ids", (value.group_id for value in self.condition_groups))
        _unique("context field ids", (value.field_id for value in self.context_schema))
        _unique("rationale ids", self.rationale_condition_ids)
        _unique("precedence ids", self.simultaneous_precedence)
        if not self.lifecycle_states or any(not state.strip() for state in self.lifecycle_states):
            raise PatternDefinitionError("lifecycle states must be non-empty")
        states = set(self.lifecycle_states)
        conditions = {item for group in self.condition_groups for item in group.condition_ids}
    ...
    def resolve_parameters(self, overrides: Mapping[str, object] | None = None) -> Mapping[str, object]:
        ...
        return MappingProxyType({key: spec.normalize(supplied.get(key, spec.default)) for key, spec in specs.items()})

    def canonical_dict(self, *, semantic_only: bool = False) -> dict[str, object]:
        result: dict[str, object] = {
            "pattern_id": self.pattern_id,
            "pattern_version": self.pattern_version,
            "required_components": sorted(self.required_components),
            "required_market_events": sorted(self.required_market_events),
            "parameters": [value.canonical(include_default=not semantic_only, include_description=not semantic_only) for value in sorted(self.parameters, key=lambda item: item.parameter_id)],
            "lifecycle_states": list(self.lifecycle_states),
            "transitions": [vars_like(value) for value in self.transitions],
            "condition_groups": [{"group_id": value.group_id, "condition_ids": list(value.condition_ids)} for value in self.condition_groups],
        ...
        if not semantic_only:
            result.update(name=self.name, description=self.description)
        return result
    ...
    def semantic_fingerprint(self) -> str:
        payload = json.dumps(self.canonical_dict(semantic_only=True), sort_keys=True, separators=(",", ":")).encode()
        return hashlib.sha256(payload).hexdigest()


class PatternDefinitionRegistry:
    def __init__(self) -> None:
        self._items: dict[tuple[str, str], PatternDefinition] = {}
    ...
    def register(self, definition: PatternDefinition) -> None:
        ...
        if existing is not None and existing.canonical_json() != definition.canonical_json():
            raise PatternDefinitionError("pattern id/version cannot map to two different definitions")
        self._items[definition.identity] = definition

    def definitions(self) -> tuple[PatternDefinition, ...]:
        return tuple(self._items[key] for key in sorted(self._items))

    def get(self, pattern_id: str, pattern_version: str) -> PatternDefinition:
        try:
            return self._items[(pattern_id, pattern_version)]
        except KeyError as exc:
            raise PatternDefinitionError(f"unknown pattern: {pattern_id}@{pattern_version}") from exc


def assert_semantic_change_is_versioned(previous: PatternDefinition, candidate: PatternDefinition) -> None:
    if previous.pattern_id != candidate.pattern_id:
        raise PatternDefinitionError("cannot compare different pattern ids")
    if previous.pattern_version == candidate.pattern_version and previous.semantic_fingerprint() != candidate.semantic_fingerprint():
        raise PatternDefinitionError("material pattern semantics changed without a pattern_version change")
